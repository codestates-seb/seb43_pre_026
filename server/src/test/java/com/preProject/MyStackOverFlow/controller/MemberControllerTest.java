...
//        long memberId = 1L;
//
//        MemberAnswerDto.Response answerResponse = MemberAnswerDto.Response.builder()
//                .boardId(1L)
//                .title("제목 1")
//                .build();
//        List<MemberAnswerDto.Response> memberAnswers = new ArrayList<>();
//        memberAnswers.add(answerResponse);
//
//        MemberBoardDto.Response boardResponse = MemberBoardDto.Response.builder()
//                .boardId(1L)
//                .title("제목 1")
//                .build();
//        List<MemberBoardDto.Response> memberBoards = new ArrayList<>();
//        memberBoards.add(boardResponse);
//
//
//        MemberDto.MemberResponse response = new MemberDto.MemberResponse();
//        response.setMemberName("김테스트");
//        response.setMemberAnswers(memberAnswers);
//        response.setMemberBoards(memberBoards);
//
//        given(memberService.findMember(Mockito.anyLong()))
//                .willReturn(new Member());
//        given(mapper.memberToMemberResponse2(Mockito.any(Member.class)))
//                .willReturn(response);
//
//        ResultActions actions =
//                mockMvc.perform(
...
//                .andExpect(jsonPath("$.data.memberAnswers[0].boardId").value(response.getMemberAnswers().get(0).getBoardId()))
//                .andExpect(jsonPath("$.data.memberAnswers[0].title").value(response.getMemberAnswers().get(0).getTitle()))
//                .andDo(document(
//                        "get-member",
//                        getRequestPreProcessor(),
//                        getResponsePreProcessor(),
...
//                                        fieldWithPath("data").type(JsonFieldType.OBJECT).description("결과 데이터"),
//                                        fieldWithPath("data.memberName").type(JsonFieldType.STRING).description("회원 성함"),
//                                        fieldWithPath("data.memberAnswers[].boardId").type(JsonFieldType.NUMBER).description("회원이 작성한 답변의 게시글 ID"),
//                                        fieldWithPath("data.memberAnswers[].title").type(JsonFieldType.STRING).description("회원이 작성한 답변의 제목"),
//                                        fieldWithPath("data.memberBoards[].boardId").type(JsonFieldType.NUMBER).description("회원이 작성한 게시글의 게시글 ID"),
//                                        fieldWithPath("data.memberBoards[].title").type(JsonFieldType.STRING).description("회원이 작성한 게시글의 제목")
//                                )
//                        )
//                ));
//    }
